...
const onAddCatalogSubmit = () => {
  ...
  addCatalogFormRef.value.validate((valid) => {
    ...
    // 获取最后一个一级目录对象 ++ let lastCatalog = catalogs.value[catalogs.value.length - 1]
    let lastCatalog = 1;
    if(catalogs.value.length != 0){
      lastCatalog = catalogs.value[catalogs.value.length - 1].sort + 1;
    }
    //++
    ...
    let newCatalog = {
      ...
      editing: false,
      level: 1,
      sort: lastCatalog, // 最后一个一级目录的 sort 值加一 sort: lastCatalog.sort + 1,
      children: [],
    };
    // 添加新目录
...
function getTableData() {
  ...
  getArticlePageList({ current: current.value, size: size.value, startDate: startDate.value, endDate: endDate.value, title: searchArticleTitle.value, type: 1 })
    .then((res) => {
      if (res.success == true) {
