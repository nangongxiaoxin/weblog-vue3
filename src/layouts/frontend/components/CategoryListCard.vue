    ...
    <div v-if="categories.length > 1"
        ...
        <div class="text-sm font-medium text-gray-600 bg-white rounded-lg dark:bg-gray-800 dark:border-gray-600 dark:text-white">
            <a @click="goCategoryArticleListPage(category.id, category.name)" v-for="(category, index) in categories"
                :key="index"
                class="dark:hover:bg-green-950 flex items-center w-full px-4 py-2 rounded-lg border-gray-200 cursor-pointer hover:bg-gray-100 hover:text-sky-600 dark:border-gray-600 dark:hover:bg-gray-600 dark:hover:text-white dark:focus:ring-gray-500 dark:focus:text-white">
